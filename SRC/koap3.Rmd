...
```{r}
perc_change$comp = NA
for (i in 1:length(perc_change$perc)) {
  if (isTRUE(perc_change$perc[i] >= perc_change$perc[96])) {
    perc_change$comp[i] = "Less or Equal"
  } else {
    perc_change$comp[i] = "More"
  }
}
```

```{r}
perc_change %>% ggplot(aes(comp, fill = continent)) + geom_bar()
```
